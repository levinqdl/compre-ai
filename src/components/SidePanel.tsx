...
export default function SidePanel({
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Translation service error"
      );
    } finally {
  ...
  return (
    <div
      className={`compre-ai-side-panel flex flex-col fixed bg-white border-gray-200 shadow-2xl z-[2147483647] font-sans overflow-y-auto ${positionClasses}`}
    >
            ...
            {(result.detectedLanguage || result.targetLanguage) && (
              <div className="text-xs text-gray-600 mb-2.5">
                {result.detectedLanguage &&
                  result.detectedLanguage.toLowerCase() !== "unknown" && (
                    <span>Detected: {result.detectedLanguage}</span>
                  )}
                {result.detectedLanguage && result.targetLanguage && " • "}
                {result.targetLanguage && (
                  <span>Target: {result.targetLanguage}</span>
                )}
              </div>
            )}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 text-gray-800 leading-normal break-words">
                  {result.explanations.map((item, index) => (
                    <div
                      key={index}
                      className={`mb-3 pb-3 ${
                        index < result.explanations!.length - 1
